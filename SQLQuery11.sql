...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Validare număr licență duplicat
        IF @NumarLicenta IS NOT NULL AND EXISTS (SELECT 1 FROM PersonalMedical WHERE NumarLicenta = @NumarLicenta AND EsteActiv = 1)
        BEGIN
            RAISERROR('Un membru al personalului cu acest număr de licență există deja în sistem.', 16, 1);
            RETURN;
        END
        ...
        BEGIN
            RAISERROR('Categoria specificată nu există.', 16, 1);
            RETURN;
        END
        ...
        BEGIN
            RAISERROR('Specializarea specificată nu există.', 16, 1);
            RETURN;
        END
        ...
        BEGIN
            RAISERROR('Subspecializarea specificată nu există.', 16, 1);
            RETURN;
        END
...
BEGIN
    ...
    BEGIN TRY
        -- Verifică că personalul există
        IF NOT EXISTS (SELECT 1 FROM PersonalMedical WHERE PersonalID = @PersonalID)
        BEGIN
            RAISERROR('Personalul medical specificat nu există.', 16, 1);
            RETURN;
        END
        ...
        -- Validare număr licență duplicat (exclus înregistrarea curentă)
        ...
        BEGIN
            RAISERROR('Un alt membru al personalului cu acest număr de licență există deja în sistem.', 16, 1);
            RETURN;
        END
...
BEGIN
    ...
    SELECT 
        ...
        pm.Departament,
        pm.Specializare,
        -- Nume din ierarhie pentru afișare
        cat.Nume AS CategorieNume,
        spec.Nume AS SpecializareNume,
        sub.Nume AS SubspecializareNume
    FROM PersonalMedical pm WITH (NOLOCK)
...
PRINT '';
PRINT '=== ACTUALIZARE STORED PROCEDURES COMPLETĂ ===';
PRINT '';
PRINT 'Proceduri actualizate:';
PRINT '✓ sp_PersonalMedical_GetPaged - suport pentru filtrare pe ierarhie';
PRINT '✓ sp_PersonalMedical_Create - acceptă noile coloane ierarhice';
PRINT '✓ sp_PersonalMedical_Update - actualizează noile coloane ierarhice';
PRINT '✓ sp_PersonalMedical_GetById - returnează datele ierarhice';
PRINT '';
...
PRINT '';
PRINT 'Următorul pas: Actualizarea aplicației Blazor pentru dropdown-urile ierarhice.';